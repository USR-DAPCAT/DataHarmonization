...
C_EXPOSATS<-C_EXPOSATS%>% left_join(dt_problemes_2018) %>% 
  select(-dtindex)
...
C_EXPOSATS<-C_EXPOSATS %>% mutate(exclusio1_prev_2018=ifelse(DG05.exclude==0,0,1)) 
...
# [DIA INDEX SUPERIOR AL 2005, ENTRADA A LA B.D ANTERIOR AL 31.12.2018 i no tenir Malalties.Exclusio anterior 2018]
...
# 2. D’aquesta cohort (T2C), traieu els pacients amb els codis del full “prevalent” o “càncer” ------------------
...
dt_matching<-dt_matching%>%mutate(DG05.cancer=ifelse(
  is.na(DG05.cancer)  | 
    DG05.cancer==0,0,1))

dt_matching<-dt_matching%>%mutate(DG05.prevalent_CVD=ifelse(
  is.na(DG05.prevalent_CVD)  | 
    DG05.prevalent_CVD==0,0,1))

dt_matching<-dt_matching%>%mutate(DG05.prevalent_KD=ifelse(
  is.na(DG05.prevalent_KD) | 
    DG05.prevalent_KD==0 ,0,1))

dt_matching<-dt_matching%>%mutate(DG05.prevalent_MET=ifelse(
  is.na(DG05.prevalent_MET) | 
    DG05.prevalent_MET==0  ,0,1))
...
dt_matching<-dt_matching %>% mutate(exc_cancer=ifelse(DG05.cancer==0,0,1)) 
dt_matching<-dt_matching %>% mutate(exc_prev_CVD=ifelse(DG05.prevalent_CVD==0,0,1)) 
dt_matching<-dt_matching %>% mutate(exc_prev_KD=ifelse(DG05.prevalent_KD==0,0,1)) 
dt_matching<-dt_matching %>% mutate(exc_prev_MET=ifelse(DG05.prevalent_MET==0,0,1)) 
...
#   preguntar JORDI.REAL

#   remove patients with any codes in sheet “non-exposed pool” before the end of the study (31/12/2018),
#   to get the candidate non-exposed patients (CNE).

#3. De tota la base de dades (ED), elimineu els pacients amb els codis del sheet “non-exposed pool”
...
dt_index_match <-dt_index_match %>%mutate(exc_prev_MET=0)
